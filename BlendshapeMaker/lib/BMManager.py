...
class MyMesh():
    ...
    def LoadMesh(self):
        ...
        for line in f:
            ...
            if (words[0] == "f"):
                self.Faces[idx_f] = np.hstack(
                    (np.array([words[1].split("/"), words[2].split("/"), words[3].split("/")]).reshape(6), 0))
...
class Mesh_VFN():
    def __init__(self, Vertices, Faces):
        ...
        for i in range(0, len(Faces)):
            cross = np.cross(Vertices[Faces[i, 2], 0:3] - Vertices[Faces[i, 0], 0:3],
                             Vertices[Faces[i, 4], 0:3]-Vertices[Faces[i, 0], 0:3])
            self.Normals[i] = cross / LA.norm(cross)


class dtf():
    def __init__(self, corrList, SOURCE_LANDMARKS, AVATAR_LANDMARKS, SourceExpression, SourceNeutral, AvatarNeutral,
                 PartsSize, PartsRot, Ed_A, Ed_ATA, Es_ATA, Es_ATc, Ei_ATA, Ei_ATc, backlandmark_list):
    ...
    def deformation_transfer(self):
        ...
        for i in range(0, len(self.SN_Faces)):
            a = np.array(
                [self.SN_Vertices[self.SN_Faces[i, 2], 0:3] - self.SN_Vertices[self.SN_Faces[i, 0], 0:3]])  # v2 - v1
            b = np.array(
                [self.SN_Vertices[self.SN_Faces[i, 4], 0:3] - self.SN_Vertices[self.SN_Faces[i, 0], 0:3]])  # v3 - v1
            cross = np.cross(a, b)
            ...
            a = np.array(
                [self.SE_Vertices[self.SE_Faces[i, 2], 0:3] - self.SE_Vertices[self.SE_Faces[i, 0], 0:3]])  # v2 - v1
            b = np.array(
                [self.SE_Vertices[self.SE_Faces[i, 4], 0:3] - self.SE_Vertices[self.SE_Faces[i, 0], 0:3]])  # v3 - v1
            cross = np.cross(a, b)
            c = cross / sqrt(LA.norm(cross))  # v4 - v1
        ...
        def MakeEl_ATA_ATc():
            ...
            El_A = sp.lil_matrix((len(self.SOURCE_LANDMARKS)*3, len(self.AN_Vertices)
                                  * 3 + len(self.AN_Faces)*3), dtype=np.float32)
            ...
            for i in range(len(self.SOURCE_LANDMARKS)):
                vector_s = self.SE_Vertices[self.SOURCE_LANDMARKS[i],
                                            0:3] - self.SN_Vertices[self.SOURCE_LANDMARKS[i], 0:3]
                if(0 <= i <= 4):
                    El_cVector[0, i*3:i*3+3] = \
                        np.dot(self.PartsRot[0], vector_s)*self.PartsSize[0] + self.AN_Vertices[self.AVATAR_LANDMARKS[i], 0:3]
                elif(5 <= i <= 9):
                    El_cVector[0, i*3:i*3+3] = \
                        np.dot(self.PartsRot[1], vector_s)*self.PartsSize[1] + self.AN_Vertices[self.AVATAR_LANDMARKS[i], 0:3]
                elif(10 <= i <= 18):
                    El_cVector[0, i*3:i*3+3] = \
                        np.dot(self.PartsRot[2], vector_s)*self.PartsSize[2] + self.AN_Vertices[self.AVATAR_LANDMARKS[i], 0:3]
                elif(19 <= i <= 24):
                    El_cVector[0, i*3:i*3+3] = \
                        np.dot(self.PartsRot[3], vector_s)*self.PartsSize[3] + self.AN_Vertices[self.AVATAR_LANDMARKS[i], 0:3]
                elif(25 <= i <= 30):
                    El_cVector[0, i*3:i*3+3] = \
                        np.dot(self.PartsRot[4], vector_s)*self.PartsSize[4] + self.AN_Vertices[self.AVATAR_LANDMARKS[i], 0:3]
                elif(31 <= i <= 50):
                    El_cVector[0, i*3:i*3+3] = \
                        np.dot(self.PartsRot[5], vector_s)*self.PartsSize[5] + self.AN_Vertices[self.AVATAR_LANDMARKS[i], 0:3]
...
class BMMng():
    ...
    def SaveMesh(self, dest, filename, Vertices, Faces, Textures, TexName, Flags):
        ...
        for i in range(len(Vertices)):
            if(self.CaredVertices[i] == True):
                ...
                f.write("%f %f %f\n" % (self.Avatar[0].Vertices[i, 0],
                                        self.Avatar[0].Vertices[i, 1],
                                        self.Avatar[0].Vertices[i, 2])
                        )
    ...
    def RescaleAvatar(self):
        ...
        self.CaredVertices = np.full(len(self.Avatar[0].Vertices), True, dtype=bool)
        self.SaveMesh("Results/", "Rescaled.ply", self.Avatar[0].Vertices, self.Avatar[0].Faces,
                      self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
    ...
    def TriangleCorrespondence(self, level):
        ...
        for k in range(len(AvatarFaces)):
            cross = np.cross(
                AvatarVertices[AvatarFaces[k, 2],
                               0: 3] - AvatarVertices[AvatarFaces[k, 0],
                                                      0: 3],
                AvatarVertices[AvatarFaces[k, 4],
                               0: 3] - AvatarVertices[AvatarFaces[k, 0],
                                                      0: 3])
        ...
        for k in range(len(SourceFaces)):
            cross = np.cross(
                SourceVertices[SourceFaces[k, 2],
                               0: 3] - SourceVertices[SourceFaces[k, 0],
                                                      0: 3],
                SourceVertices[SourceFaces[k, 4],
                               0: 3] - SourceVertices[SourceFaces[k, 0],
                                                      0: 3])
            SourceV4[k] = SourceVertices[SourceFaces[k, 0], 0:3] + cross / sqrt(LA.norm(cross))
            SourceTriangleNormals[k] = cross / LA.norm(cross)

        for i in range(len(SourceFaces)):
        ...
        def MakeEs_ATA_ATc():
            ...
            for i in range(len(self.Adjacent)):
                for j in range(len(self.Adjacent[i])):
                    adj_num += 1
        ...
        def MakeEi_ATA_ATc():
            ...
            for i in range(len(AvatarFaces)):
                Flat_identity = sp.vstack(
                    (Flat_identity, sp.identity(3, format="lil").reshape((9, 1))),
        ...
        def MakeEcons_ATA_ATc():
            ...
            for i in range(len(AvatarVertices)):
                if Used[i] is False:
                    unusedpoints += [i]
            for i in range(len(unusedpoints)):
                (Econs_A, Econs_cVector) = AddConstraints(unusedpoints[i], [0, 0, 0])
            ...
            for i in range(len(self.AVATAR_LANDMARKS)):
                (Econs_A, Econs_cVector) = AddConstraints(
                    self.AVATAR_LANDMARKS[i], SourceVertices[self.SOURCE_LANDMARKS[i], 0:3])
            # Backward landmark constraints
            if self.config[0]:
                for i in range(len(self.AVATAR_BACKWARD_LANDMARKS)):
                    (Econs_A, Econs_cVector) = AddConstraints(
                        self.AVATAR_BACKWARD_LANDMARKS[i], SourceVertices[self.SOURCE_LANDMARKS[i], 0:3])
            else:
                for i in self.SOURCE_BACKWARD_LANDMARKS:
                    norm = LA.norm(SourceVertices[i, 0:3] - AvatarVertices[:, 0:3], axis=1)
        ...
        def ClosestValidPoint(vertices, threshold):
            # Triangle index list that shares the vertex
            AvatarTrianglesList = [[] for row in range(len(AvatarVertices))]
            ...
            # index of closest valid point on the source mesh to target vertex
            Validpoint = [[] for row in range(len(AvatarVertices))]
            ...
            for k in range(0, len(AvatarFaces)):
                cross = np.cross(vertices[AvatarFaces[k, 2] * 3: AvatarFaces[k, 2] * 3 + 3] -
                                 vertices[AvatarFaces[k, 0] * 3: AvatarFaces[k, 0] * 3 + 3],
                                 vertices[AvatarFaces[k, 4] * 3: AvatarFaces[k, 4] * 3 + 3] -
                                 vertices[AvatarFaces[k, 0] * 3: AvatarFaces[k, 0] * 3 + 3])
                AvatarTriangleNormals[k] = cross / LA.norm(cross)

            # compute vertex normals
            for i in range(0, len(AvatarVertices)):
                if(len(AvatarTrianglesList[i]) == 0):
                    continue
                AvatarN = AvatarTriangleNormals[AvatarTrianglesList[i][0]]
                for j in range(1, len(AvatarTrianglesList[i])):
        ...
        self.SaveMesh(
            "Results/", "iteration_1.ply", self.Result_iteration[0],
            self.Avatar[0].Faces, self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
        ...
        self.SaveMesh(
            "Results/", "iteration_2.ply", self.Result_iteration[1],
            self.Avatar[0].Faces, self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
        ...
        self.SaveMesh(
            "Results/", "iteration_3.ply", self.Result_iteration[2],
            self.Avatar[0].Faces, self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
        ...
        self.SaveMesh(
            "Results/", "iteration_4.ply", self.Result_iteration[3],
            self.Avatar[0].Faces, self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
        ...
        for i in range(len(AvatarFaces)):
            ResultTriangleCentroid[i] = (
                ResultVertices[AvatarFaces[i, 0]] + ResultVertices[AvatarFaces[i, 2]] +
                ResultVertices[AvatarFaces[i, 4]]) / 3
        for i in range(len(SourceFaces)):
            SourceTriangleCentroid[i] = (
                SourceVertices[SourceFaces[i, 0],
                               0: 3] + SourceVertices[SourceFaces[i, 2],
                                                      0: 3] + SourceVertices[SourceFaces[i, 4],
                                                                             0: 3]) / 3

        # compute Result triangle normals
        for i in range(len(AvatarFaces)):
            cross = np.cross(
                ResultVertices[AvatarFaces[i, 2]] - ResultVertices[AvatarFaces[i, 0]],
                ResultVertices[AvatarFaces[i, 4]] - ResultVertices[AvatarFaces[i, 0]])
            ResultTriangleNormals[i] = cross / LA.norm(cross)

        corr = []
        # Hold the number of corresponding source triangles
        num_corrS_triangles = np.zeros(len(AvatarFaces), dtype="int")
        ...
        def SplitTriangles(Vertices, Faces, Textures, num_corrS_triangles):
            ...
            def delaunay_split(V, Face, Texture, density_mode):
                ...
                if(normal[0] < 0):
                    angle_y = -angle_y
                Rot = np.dot([[cos(angle_y), 0, sin(angle_y)], [0, 1, 0], [-sin(angle_y), 0, cos(angle_y)]],
                             [[1., 0., 0.], [0, np.cos(angle_x), -sin(angle_x)], [0, sin(angle_x), cos(angle_x)]])
                z = np.dot(Rot.T, V[Face[0], 0:3])[2]
                v_coords = np.hstack([np.dot(Rot.T, V[Face[0], 0:3])[0:2], np.dot(
                    Rot.T, V[Face[2], 0:3])[0:2], np.dot(Rot.T, V[Face[4], 0:3])[0:2]])
                ...
                for i in range(3, len(points)):
                    AddVertices = np.vstack((AddVertices, np.hstack(
                        [np.dot(Rot, np.hstack((points[i], z))), new_idx_v+i-3])))
                    idxlist += [new_idx_v+i-3]
                for i in range(len(simplices)):
                    AddFaces = np.vstack(
                        (AddFaces,
                         [idxlist[simplices[i, 0]],
                          0, idxlist[simplices[i, 1]],
                          0, idxlist[simplices[i, 2]],
                          0, Face[6]]))
                    AddTextures = np.vstack(
                        (AddTextures,
                         [tex[simplices[i, 0],
                              0],
                          tex[simplices[i, 0],
        ...
        def SplitTriangles_2(Vertices, Faces, Textures, num_corrS_triangles):
            for i in range(len(Faces)):
                if(4 <= num_corrS_triangles[i]):
                    ...
                    if(Faces[adj, 0] != idx1 and Faces[adj, 0] != idx2):
                        idx3_adj = Faces[adj, 0]
                        tex3_adj = Textures[adj, 0:2]
                    elif(Faces[adj, 2] != idx1 and Faces[adj, 2] != idx2):
                        idx3_adj = Faces[adj, 2]
                        tex3_adj = Textures[adj, 2:4]
                    elif(Faces[adj, 4] != idx1 and Faces[adj, 4] != idx2):
                        idx3_adj = Faces[adj, 4]
                        tex3_adj = Textures[adj, 4:6]
                    ...
                    AddFaces = np.vstack(
                        (AddFaces, np.array([idx3_adj, 0, new_idx_v, 0, idx2, 0, Faces[adj, 6]], dtype=np.int32)))
        ...
        if(max(num_corrS_triangles) >= 10 and level < 0 and len(SourceFaces)/2 > len(self.Avatar[0].Faces)):
            ...
            self.Avatar[0].Vertices, self.Avatar[0].Faces, self.Avatar[0].Textures = SplitTriangles_2(
                self.Avatar[0].Vertices, self.Avatar[0].Faces, self.Avatar[0].Textures, num_corrS_triangles)
            print("done (%f sec)" % (time.time() - start_time))
        ...
        for i in range(len(self.Avatar[0].Faces)):
            if(Used[i]==False): 
                unusedfaces += [i]
        ...
        self.SaveMesh("Results/", "Splitted.ply", self.Avatar[0].Vertices, self.Avatar[0].Faces,
                      self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
    ...
    def DeformationTransfer(self):
        ...
        def SN_Energy(args):
            RotX = np.array([[1, 0, 0], [0, np.cos(args[0]), -np.sin(args[0])],
                             [0, np.sin(args[0]), np.cos(args[0])]], dtype=np.float32)
            RotY = np.array([[np.cos(args[1]), 0, np.sin(args[1])], [0, 1, 0],
                             [-np.sin(args[1]), 0, np.cos(args[1])]], dtype=np.float32)
            normal = np.dot(np.dot(RotY, RotX), np.array([0, 0, 1]))
            normal = normal/LA.norm(normal)
            E = []
            for i in range(len(cared_landmarks)):
                for j in range(i, len(cared_landmarks)):
                    E += [np.dot(normal, SN_Vertices[self.SOURCE_LANDMARKS[cared_landmarks[j]],
                                                     0:3]-SN_Vertices[self.SOURCE_LANDMARKS[cared_landmarks[i]], 0:3])]
            return E
        ...
        def AN_Energy(args):
            ...
            for i in range(len(cared_landmarks)):
                for j in range(i, len(cared_landmarks)):
                    E += [np.dot(normal, AN_Vertices[self.AVATAR_LANDMARKS[cared_landmarks[j]],
                                                     0:3]-AN_Vertices[self.AVATAR_LANDMARKS[cared_landmarks[i]], 0:3])]
            return E
        ...
        def SN_partsnormals():
            ...
            vlist = vlist-vc
            for i in range(len(cared_landmarks)-1):
        ...
        for i in range(6):
            angle_x = PartsAngles_S[i, 0]
            angle_y = PartsAngles_S[i, 1]
            RotX = np.array([[1, 0, 0], [0, np.cos(angle_x), -np.sin(angle_x)],
                             [0, np.sin(angle_x), np.cos(angle_x)]], dtype=np.float32)
            RotY = np.array([[np.cos(angle_y), 0, np.sin(angle_y)], [0, 1, 0],
                             [-np.sin(angle_y), 0, np.cos(angle_y)]], dtype=np.float32)
            Rot_S = np.dot(RotY, RotX)
            angle_x = PartsAngles_A[i, 0]
            angle_y = PartsAngles_A[i, 1]
            RotX = np.array([[1, 0, 0], [0, np.cos(angle_x), -np.sin(angle_x)],
                             [0, np.sin(angle_x), np.cos(angle_x)]], dtype=np.float32)
            RotY = np.array([[np.cos(angle_y), 0, np.sin(angle_y)], [0, 1, 0],
                             [-np.sin(angle_y), 0, np.cos(angle_y)]], dtype=np.float32)
        ...
        def Measure_Source_Parts_Size(lower, upper, partsnum):
            xs = np.dot(PartsRot_S[partsnum], SN_Vertices[self.SOURCE_LANDMARKS[lower], 0:3])[0]
            xl = xs
            ys = np.dot(PartsRot_S[partsnum], SN_Vertices[self.SOURCE_LANDMARKS[lower], 0:3])[1]
            yl = ys
            zs = np.dot(PartsRot_S[partsnum], SN_Vertices[self.SOURCE_LANDMARKS[lower], 0:3])[2]
            zl = zs
        ...
        def Measure_Avatar_Parts_Size(lower, upper, partsnum):
            xs = np.dot(PartsRot_A[partsnum], AN_Vertices[self.AVATAR_LANDMARKS[lower], 0:3])[0]
            xl = xs
            ys = np.dot(PartsRot_A[partsnum], AN_Vertices[self.AVATAR_LANDMARKS[lower], 0:3])[1]
            yl = ys
            zs = np.dot(PartsRot_A[partsnum], AN_Vertices[self.AVATAR_LANDMARKS[lower], 0:3])[2]
            zl = zs
        ...
        A_PartsSize = np.array([[x, y, z]])
        print("Right eyebrow: x: %f, y: %f, z: %f" % (x, y, z))
        (x, y, z) = Measure_Avatar_Parts_Size(5, 9, 1)
        A_PartsSize = np.vstack((A_PartsSize, [[x, y, z]]))
        print("Left eyebrow: x: %f, y: %f, z: %f" % (x, y, z))
        ...
        elapsed_time = time.time() - start_time
        print("Ed_A, Ed_ATA :", elapsed_time)
        ...
        i = 0
        currMesh = dtf(
            self.Correspondences, self.SOURCE_LANDMARKS, self.AVATAR_LANDMARKS, self.BlendShapes[0],
            self.BlendShapes[0],
            self.Avatar[0],
            PartsSize, PartsRot, Ed_A, Ed_ATA, self.Es_ATA, self.Es_ATc, self.Ei_ATA, self.Ei_ATc, self.
            backlandmark_list)
        print("\n\nGenerating Neutral AvatarMesh")
        currMesh.deformation_transfer()
        self.SaveMesh("Results/", "AvatarMesh_Neutral.ply", currMesh.Vertices,
                      self.Avatar[0].Faces, self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)

        for meshnum in range(48):
            if ((meshnum > 1 and meshnum < 14) or meshnum == 17 or meshnum == 18 or meshnum == 19 or meshnum == 22 or meshnum == 26
                ...
                currMesh = dtf(self.Correspondences, self.SOURCE_LANDMARKS, self.AVATAR_LANDMARKS, self.BlendShapes
                               [i + 1],
                               self.BlendShapes[0],
                               self.Avatar[0],
                               PartsSize, PartsRot, Ed_A, Ed_ATA, self.Es_ATA, self.Es_ATc, self.Ei_ATA, self.Ei_ATc,
                               self.backlandmark_list)
            print("\n\nGenerating Avatar blendshape ", meshnum)
            currMesh.deformation_transfer()
            self.Avatar.append(currMesh)
            name = "AvatarMesh_%d.ply" % (meshnum)
            self.SaveMesh("Results/", name, self.Avatar[i+1].Vertices, self.Avatar[0].Faces,
                          self.Avatar[0].Textures, self.Avatar[0].TexName, self.Avatar[0].Flags)
            i += 1
